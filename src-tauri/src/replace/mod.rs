...
impl ReplaceService {
    ...
    pub(crate) fn replace_selected_text(&self, replacement: &str) -> ReplaceResult {
        let _apartment = match ComApartment::initialize() {
            Ok(apartment) => apartment,
            Err(error) => return ReplaceResult::Failure(ReplaceError::Com(error)),
        };

        match self.preferred.replace_selected_text(replacement) {
            ReplaceResult::Unsupported => self.fallback.replace_selected_text(replacement),
            ReplaceResult::Failure(preferred_error) => {
                match self.fallback.replace_selected_text(replacement) {
